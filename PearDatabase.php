...
include_once('config/config.enable_sqllog.php');
...
class PearDatabase{
	...
	function checkSlaveConnection() {
		if(!isset($this->slavedatabase)) {
		    $this->println("TRANS creating new connection");
        ...
        function checkReportConnection() {
		if(!isset($this->reportdatabase)) {
		    $this->println("TRANS creating new connection");
		    $this->reportConnect(false);
		} else {
		    //$this->println("checkconnect using old connection");
		}
	}
        function checkReportMainConnection() {
		if(!isset($this->reportmaindatabase)) {
		    $this->println("TRANS creating new connection");
		    $this->reportMainConnect(false);
		} else {
		    //$this->println("checkconnect using old connection");
		}
	}
	...
	$_REQUEST['procedure'] = 4;
	$sql_start_time = microtime(true);
	$result = & $this->database->Execute($sql);
	$this->logSqlTiming($sql_start_time, microtime(true), $sql);
	
	if($_COOKIE['qdebug']=='TRUE')
			$this->database->Execute("INSERT INTO `sify_sql_error_log` (`error`) VALUES (?)",array($msg.' Query Executed:' . $sql  .'[' . implode(",", $params) . ']'));
        //echo "<br/>".$sql."<br/>";
        
	$this->lastmysqlrow = -1;
	if(!$result){
         $this->checkError($msg.' Query Failed:' . $sql . '::', $dieOnError);
                   $this->database->Execute("INSERT INTO `sify_sql_error_log` (`error`) VALUES (?)",array($msg.' Query Failed:' . $sql  .'[' . implode(",", $params) . ']'));  
	...
	function forum_query($sql){
		$forumResult = mysql_query($sql);
		while($forumRowResult = mysql_fetch_array($forumResult)){
			$forumArray[]= $forumRowResult[0]; 
		}
		return $forumArray;
	}
   	...
   	*/	
	function pquery($sql, $params, $dieOnError=false, $msg='',$callcount=0,$seq='') {
		global $log, $default_charset,$slaveconfig;
                
                if($seq=='')
                    $seq=rand(100000,900009);
		...
		$read_operation = preg_match( '/^SELECT/i', $sql );
		if ($read_operation && count($slaveconfig)>0) :
			$this->checkSlaveConnection();
		else :
			$this->checkConnection();
		endif;
		
                
                //echo $this->slavedatabase->host;exit;
                //print_r($this->slavedatabase);exit;
		...
		if (count($params) > 0) {
            ...
            $_REQUEST['procedure'] = 1;
            if($this->avoidPreparedSql || empty($params)) {
			$sql = $this->convert2Sql($sql, $params);
			if($read_operation && count($slaveconfig)>0):
				$result = &$this->slavedatabase->Execute($sql);
			else :
				$result = &$this->database->Execute($sql);
			endif;
		} else {
			if($read_operation && count($slaveconfig)>0):
				$result = &$this->slavedatabase->Execute($sql, $params);
			else:
		...
		if(!$result){$this->checkError($msg.' Query Failed:' . $sql . '::', $dieOnError);
                    $result+=$msg.' Query Failed:' . $sql  .'[' . implode(",", $params) . ']';
					$this->database->Execute("INSERT INTO `sify_sql_error_log` (`error`) VALUES (?)",array($seq.' ::: '.$msg.' Query Failed:' . $sql  .'[' . implode(",", $params) . ']'));				
                    //print_r($msg);
                    //print "<br/>  ERROR : ".$sql.' Params :  [' . implode(",", $params) . ']';
                    //exit;
		}
		//print_r($sql);
		...
		if (!is_object($result) && $callcount<2)
		{
				$callcount++;
                                if($read_operation)
                                    $this->database->Execute("INSERT INTO `sify_sql_error_log` (`error`) VALUES (?)",array($seq.' ::: '.$msg.' Query Executing Again :'.($callcount+1).'  Time : In Host :'.$this->slavedatabase->host.' : With'. $sql  .'[' . implode(",", $params) . ']'));
				else
                                    $this->database->Execute("INSERT INTO `sify_sql_error_log` (`error`) VALUES (?)",array($seq.' ::: '.$msg.' Query Executing Again :'.($callcount+1).'  Time : In Host :'.$this->database->host.' : With'. $sql  .'[' . implode(",", $params) . ']'));
                                $result=$this->pquery($sql, $params, $dieOnError, $msg,$callcount,$seq);
		}
   	...
   	* @param $msg -- Error message on query execution failure
   	*/	
	function rquery($sql, $params, $dieOnError=false, $msg='',$callcount=0,$seq='') {
		global $log, $default_charset,$rptconfig;
		...
		$log->debug('Prepared sql query being executed : '.$sql);
		$read_operation = preg_match( '/^SELECT/i', $sql );
		if ($read_operation && count($rptconfig)>0) :
			$this->checkReportConnection();
		else :
			$this->checkConnection();
		endif;
		...
		if (count($params) > 0) {
            ...
            $_REQUEST['procedure'] = 3;
            if($this->avoidPreparedSql || empty($params)) {
			$sql = $this->convert2Sql($sql, $params);
			if($read_operation && count($rptconfig)>0):
				$result = &$this->reportdatabase->Execute($sql);
			else :
				$result = &$this->database->Execute($sql);
			endif;
		} else {
			if($read_operation && count($rptconfig)>0):
				$result = &$this->reportdatabase->Execute($sql, $params);
			else:
				$result = &$this->database->Execute($sql, $params);
			endif;
		}
		...
		if (!is_object($result) && $callcount<2)
		{
				$callcount++;
				$this->database->Execute("INSERT INTO `sify_sql_error_log` (`error`) VALUES (?)",array($seq.' ::: '.$msg.' Query Executing Again :'.($callcount+1).'  Time : In Host :'.$this->slavedatabase->host.' : With'. $sql  .'[' . implode(",", $params) . ']'));
				$result=$this->rquery($sql, $params, $dieOnError, $msg,$callcount,$seq);
		}
   	...
   	* @param $msg -- Error message on query execution failure
   	*/	
	function raquery($sql, $params, $dieOnError=false, $msg='') {
		if($_REQUEST['source']=='NONREPORT')
		...
		$read_operation = preg_match( '/^SELECT/i', $sql );
		if ($read_operation && count($rptconfig)>0) :
			$this->checkReportMainConnection();
		else :
			$this->checkConnection();
		endif;
		...
		if (count($params) > 0) {
			...
			$sql = $this->convert2Sql($sql, $params);
			if($read_operation && count($rptconfig)>0):
				$result = &$this->reportmaindatabase->Execute($sql);
			else :
				$result = &$this->database->Execute($sql);
			endif;
		} else {
			if($read_operation && count($rptconfig)>0):
				$result = &$this->reportmaindatabase->Execute($sql, $params);
			else:
				$result = &$this->database->Execute($sql, $params);
			endif;
		}
	...
	function mquery($sql, $params, $dieOnError=false, $msg='',$callcount=0,$seq='') {
		global $log, $default_charset,$slaveconfig;
		...
		if($read_operation && !is_object($result) && $callcount<2)
		{
				$callcount++;
				$this->database->Execute("INSERT INTO `sify_sql_error_log` (`error`) VALUES (?)",array($seq.' ::: '.$msg.' Query Executing Again :'.($callcount+1).'  Time : In Host :'.$this->database->host.' : With'. $sql  .'[' . implode(",", $params) . ']'));
                                $result=$this->mquery($sql, $params, $dieOnError, $msg,$callcount,$seq);
		}
    ...
    /**
     * Usage:- Not need to write for loop each time [ Build array format result ]
     * pquery or mquery result variable just pass as argument this function &
     * This Function return Array format
     */
    function getResultSet($result){
        $multiArr = array();
        $cnt = $this->num_rows($result);
        for ($index = 0; $index < $cnt; $index++) {
                foreach($result->fields as $key=>$single){
                if(is_numeric($key)){
                    // Do Nothing.....
                } else {
                    $multiArr[$index][$key] = $this->query_result($result, $index, $key);
                }
            }
        }
        return $multiArr;
    ...
    function fetchByAssoc(&$result, $rowNum = -1, $encode=true) {
		...
		if(isset($result) && is_object($result) && $rowNum < 0) {
	...
	function slaveConnect($dieOnError = false) {
		global $dbconfigoption,$dbconfig, $slaveconfig;
		if(!isset($this->dbType)) {
		    $this->println("ADODB Connect : DBType not specified");
		    return;
		}
		$this->slavedatabase = ADONewConnection($this->dbType);
		$slaveCount = count($slaveconfig);
		$slaveconfig = $slaveconfig['slave_'.rand(1,$slaveCount)];
		
		$this->slavedatabase->Connect($slaveconfig['db_hostname'], $slaveconfig['db_username'], $slaveconfig['db_password'], $slaveconfig['db_name'], true);
		$this->slavedatabase->LogSQL($this->enableSQLlog);
    ...
    function reportConnect($dieOnError = false) {
		global $dbconfigoption,$dbconfig, $rptconfig;
		if(!isset($this->dbType)) {
		    $this->println("ADODB Connect : DBType not specified");
		    return;
		}
		$this->reportdatabase = ADONewConnection($this->dbType);
		$slaveCount = count($rptconfig);
		$rptconfig = $rptconfig['report_'.rand(1,$slaveCount)];
		
		$this->reportdatabase->Connect($rptconfig['db_hostname'], $rptconfig['db_username'], $rptconfig['db_password'], $rptconfig['db_name'], true);
		$this->reportdatabase->LogSQL($this->enableSQLlog);
    ...
    /* REPORT DATABASE CONNECTION */
    function reportMainConnect($dieOnError = false) {
		global $dbconfigoption,$dbconfig, $rptconfig;
		if(!isset($this->dbType)) {
		    $this->println("ADODB Connect : DBType not specified");
		    return;
		}
		$this->reportmaindatabase = ADONewConnection($this->dbType);
		$slaveCount = count($rptconfig);
		$rptapconfig = $rptconfig['report_'.rand(1,$slaveCount)];
		
		$this->reportmaindatabase->Connect($rptapconfig['db_hostname'], $rptapconfig['db_username'], $rptapconfig['db_password'], $rptapconfig['app_db_name'], true);
		$this->reportmaindatabase->LogSQL($this->enableSQLlog);
    ...
    function PearDatabase($dbtype='',$host='',$dbname='',$username='',$passwd='') {
		global $currentModule, $LBL_LOG_SQL_QUERIES;
                
                $this->enableSQLlog = $LBL_LOG_SQL_QUERIES;
    ...
    function disconnect() {
	    ...
	    } else if(isset($this->slavedatabase)){
			mysql_close($this->slavedatabase);
	    } else {
			$this->database->disconnect();
	    }
		unset($this->slavedatabase);
	    unset($this->database);
		}
    }
	...
	function escapeDbName($dbName='') {
        ...
        function xmlquery($query,$params,$simple=true,$table='query')
        {
            //$result=$this->pquery($query,$params);
            
            global $cus_host,$cus_user,$cus_pass,$cus_port,$cus_db;
            
            $mysqli = new mysqli($cus_host,$cus_user,$cus_pass,$cus_db);
            
            $buffer="";         
            
            $buffer.= '<results name="'.htmlspecialchars($table) .'">';
            ...
            if ($mysqli->multi_query($query)) {
                ...
                do {
                    ...
                    if ($result = $mysqli->store_result()) {
                        ...
                        $fieldsArray=$result->fetch_fields();;
                        $columns_cnt=count($fieldsArray);
                        
                        //print_r($fieldsArray[0]->name);
                        ...
                        while ($row = $result->fetch_row()) {
                            ...
                            for ($i =0;$i<$columns_cnt;$i++) {
                                ...
                                if($simple)
                                {
                                    $buffer.='      '.htmlspecialchars($fieldsArray[$i]->name).'="'.htmlspecialchars((string)$data)
                                        .'"';
                                }
                                else
                                {
                                    $buffer.='<'.htmlspecialchars($fieldsArray[$i]->name).'>'.htmlspecialchars((string)$data)
                                        .'</'.htmlspecialchars($fieldsArray[$i]->name).'>';
                                }    
...
//                                // If a cell is NULL, still export it to preserve the XML structure
//                                
//                                $data=$this->query_result($result,$k,$fieldsArray[$i]);
//                                
//                                if (!isset($data) || is_null($data)) {
//                                    $data= 'NULL';
//                                }
...
//                                if($simple)
//                                {
//                                    $buffer.='      '.htmlspecialchars($fieldsArray[$i]).'="'.htmlspecialchars((string)$data)
//                                        .'"';
//                                }
//                                else
//                                {
//                                    $buffer.='<'.htmlspecialchars($fieldsArray[$i]).'>'.htmlspecialchars((string)$data)
//                                        .'</'.htmlspecialchars($fieldsArray[$i]).'>';
//                                }    
//                                
//                            }
//                            
...
//            else
//            {
//                   $buffer.="<error>No RECORDS FOUND</error>";
//            }
//            
//            $buffer.='</results>';
            //$expire=time()+60*60*24*30;
            //setcookie("lastXMLRes", html_to_utf8($buffer),$expire);
            //echo $buffer;
            return $buffer;
        }
