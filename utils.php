...
$_REQUEST['parenttab'] = htmlspecialchars($_REQUEST['parenttab']);

/*if($_REQUEST['module']==trim("PurchaseOrder")) {
    require_once('modules/'.$_REQUEST['module'].'/utils/DetailViewUtils.php');
    require_once('modules/'.$_REQUEST['module'].'/utils/ListViewUtils.php');	
    require_once('modules/'.$_REQUEST['module'].'/utils/EditViewUtils.php');
    require_once('modules/'.$_REQUEST['module'].'/utils/PurchaseOrderUtils.php');
    require_once('modules/'.$_REQUEST['module'].'//utils/SearchUtils.php');
} else {*/
...
function get_user_array_ws($add_blank=true, $status="Active", $assigned_user="",$private="",$module="",$from=0)
{
	global $log;
	$log->debug("Entering get_user_array(".$add_blank.",". $status.",".$assigned_user.",".$private.") method ...");
	global $current_user;
	if($assigned_user != '')
	{
		require('user_privileges/sharing_privileges_'.$assigned_user.'.php');
		require('user_privileges/user_privileges_'.$assigned_user.'.php');
	}
	static $user_array = null;
	...
	if($user_array == null)
	{
		...
		if (empty($status)) {
				...
				if($private == 'private')
				{
					$log->debug("Sharing is Private. Only the current user should be listed");
					$query = "select id as id,user_name as user_name from vtiger_users where id=? and status='Active' union select vtiger_user2role.userid as id,vtiger_users.user_name as user_name from vtiger_user2role inner join vtiger_users on vtiger_users.id=vtiger_user2role.userid inner join vtiger_role on vtiger_role.roleid=vtiger_user2role.roleid where vtiger_role.parentrole like ? and status='Active' union select shareduserid as id,vtiger_users.user_name as user_name from vtiger_tmp_write_user_sharing_per inner join vtiger_users on vtiger_users.id=vtiger_tmp_write_user_sharing_per.shareduserid where status='Active' and vtiger_tmp_write_user_sharing_per.userid=? and vtiger_tmp_write_user_sharing_per.tabid=?";	
					$params = array($assigned_user, $current_user_parent_role_seq."::%", $assigned_user, getTabid($module));	
				}
				else
				{
					$log->debug("Sharing is Public. All vtiger_users should be listed");
		...
		$query .= " and id > ".$from." ORDER BY id LIMIT 0,10";
...
function get_role_array($depth,$parentrole)
{
	global $log;
	$log->debug("Entering get_role_array(".$depth.",". $parentrole.") method ...");
	global $current_user;
	if(isset($current_user) && $current_user->id != '')
	{
		require('user_privileges/sharing_privileges_'.$current_user->id.'.php');
	...
	if($user_array == null)
	{
		...
		// Including deleted vtiger_users for now.
		if (!empty($parentrole)) {
			$query1="SELECT * from vtiger_role where roleid='".$parentrole."'";
                        //echo $query1;
			$result1 = $db->pquery($query1, $params, true, "Error filling in user array: ");                        
                        $query = "SELECT * from vtiger_role where parentrole like '".$db->query_result($result1,0,'parentrole')."%' and depth>".$db->query_result($result1,0,'depth');
			//echo $query;
                        $params = array();
		}	
		else
		...
		while($row = $db->fetchByAssoc($result))
		{
			...
			$temp_result[$row['roleid']] = $depthS.$row['rolename'];
		}
...
function get_role_array_withPR($depth,$parentrole)
{
	global $log;
	$log->debug("Entering get_role_array(".$depth.",". $parentrole.") method ...");
	global $current_user;
...
//			}
			...
			//$depthS.="|-";
                        ...
                        $tempRes=array();
                        $tempRes['rolename']=$row['rolename'];
                        $tempRes['parentrole']=$row['parentrole'];
                        
			$temp_result[$row['roleid']] = $tempRes;
		}
...
function get_products_array()
{
	...
	if($user_array == null)
	{
		...
		$temp_result = Array();
		$condition="";
		//$query1="SELECT productid from  user_product where userid=".$current_user->id;
		$query="select Distinct vpc.productcategoryid,vpc.productcategory from vtiger_productcategory as vpc
                        inner join vtiger_products as vp on vpc.productcategory=vp.productcategory inner join sify_user_product as sup
                        on vp.productid=sup.productid and sup.userid=".$current_user->id;
		$params = array();
		$result = $db->pquery($query, $params, true, "Error filling in user array: ");
		
		/*while($row = $db->fetchByAssoc($result1))
		{
			if(trim($row['productid'])!="")
				$condition=trim($row['productid']);
		}
			
		$query="SELECT * from vtiger_productcategory where productcategoryid > 1";
		$params = array();
		
		if($condition!="")
		{
			$query.=" and productid in (".$condition.")";	
		}*/
		...
		while($row = $db->fetchByAssoc($result))
		{
			$temp_result[$row['productcategoryid']] = $row['productcategory'];
		}
	...
	$total_array[0]=&$user_array;
	$total_array[1]=&$role_array;
		
	$log->debug("Exiting get_products_array method ...");
	
	return $total_array;
}

function get_user_array_by_role($add_blank=true, $status="Active", $assigned_user="",$private="",$roleName)
{
	global $log;
	$log->debug("Entering get_user_array(".$add_blank.",". $status.",".$assigned_user.",".$private.") method ...");
	global $current_user;
	...
	if($user_array == null)
	{
		...
		if (empty($status)) {
				...
				else
				{
					...
					{
						$log->debug("Sharing is Public. All vtiger_users should be listed");
						$query = "SELECT * from vtiger_users where `id` in (SELECT `userid` FROM `vtiger_user2role` where `roleid` in (SELECT `roleid` FROM `vtiger_role` WHERE `rolename`=?)) and status=?";
						$params = array($roleName,$status);
					}
					else{
						$log->debug("Sharing is Public. All vtiger_users should be listed");
...
function get_group_array_ws($add_blank=true, $status="Active", $assigned_user="",$private="",$module)
{
	global $log;
	$log->debug("Entering get_user_array(".$add_blank.",". $status.",".$assigned_user.",".$private.") method ...");
	global $current_user;
...
function return_module_language($language, $module)
{
	...
	@include("modules/$module/language/$language.lang.php");
        @include("language/$module/$default_language.lang.php");
	if(!isset($mod_strings))
	{
        ...
        if(isset($mod_addon_strings))
        {
            $mod_strings=array_merge($mod_strings,$mod_addon_strings);
        }
...
function getPoStatus($po_id)
{
	...
	global $log;
        ...
        global $adb;
        $sql = "select status from vtiger_purchaseorder where purchaseorderid=?";
        $result = $adb->pquery($sql, array($po_id));
        $po_status = $adb->query_result($result,0,"status");
...
function get_days_n_dates_wdays($st,$en)
{
	global $log;
	$log->debug("Entering get_days_n_dates(".$st.",".$en.") method ...");
        $stdate_arr=explode("-",$st);
        ...
        for($i=0;$i<$days;$i++)
        {
				...
				$day_date[date("Y-m-d",mktime(0,0,0,date("$stdate_arr[1]"),(date("$stdate_arr[2]")+($i)),date("$stdate_arr[0]")))] = date("l",mktime(0,0,0,date("$stdate_arr[1]"),(date("$stdate_arr[2]")+($i)),date("$stdate_arr[0]")));
        }
        if(!isset($day_date))
                $day_date=0;
        $nodays_dates=array($days,$day_date);
...
function sify_date_diff($earlierDate, $laterDate) {
...
function getAccountImageName($id)
{
	global $log;
	$log->debug("Entering getAccountImageName(".$id.") method ...");
	global $adb;
	$query = "select cf_accounts_accounts_image from vtiger_accountscf where accountid=?";
	$result = $adb->pquery($query, array($id));
	$image_name = $adb->query_result($result,0,"cf_accounts_accounts_image");
	$log->debug("Inside getAccountImageName. The image_name is ".$image_name);
	$log->debug("Exiting getAccountImageName method ...");
	return $image_name;

}

function getCompanyImageName($id)
{
	global $log;
	$log->debug("Entering getCompanyImageName(".$id.") method ...");
	global $adb;
	$query = "select cf_company_logo from vtiger_companycf where compnayid=?";
	$result = $adb->pquery($query, array($id));
	$image_name = $adb->query_result($result,0,"cf_company_logo");
	$log->debug("Inside getCompanyImageName. The image_name is ".$image_name);
	$log->debug("Exiting getCompanyImageName method ...");
	return $image_name;
	...
	for($i=0;$i < $adb->num_rows($result);$i++)
	{
		...
		if($uitype == 15 && $field_count > 0 && ($fieldname == 'taskstatus' || $fieldname == 'eventstatus'))
		{
			...
			if($temp_count > 0)
			{
                ...
                if($uitype == 555 && $field_count > 0 && ($fieldname == 'taskstatus' || $fieldname == 'eventstatus')) //pk
		{
			$temp_count =count($temp_status[$keyvalue]);
			if($temp_count > 0)
			{
...
function getSettingsBlocks(){
	global $adb;
        ...
        $mapped = getDistributorMapping();
        $str = " where dist_setting=1";
        if(!$mapped) {
            $str = "";
        }
	$sql = "select * from vtiger_settings_blocks $str order by sequence";
...
function getSettingsFields(){
	global $adb;
        /* FRPRDINXT-774 FRPRDINXT-775 */
        $distArr = getDistrIDbyUserID();
        $user_role=fetchUserRole($_SESSION["authenticated_user_id"]);
        if($distArr['id'] > 0)
        {
            $sql = "select * from vtiger_settings_field where blockid!=? and dist_setting=1 AND roleid='".$user_role."' order by blockid,sequence";
        }
        else
        {
            $sql = "select * from vtiger_settings_field where blockid!=? and active=0 AND roleid='".$user_role."' order by blockid,sequence";
        }
...
function DeleteEntity($module,$return_module,$focus,$record,$return_id) { 
	...
	$log->debug("Entering DeleteEntity method ($module, $return_module, $record, $return_id)");
        ...
        $focus = CRMEntity::getInstance($module);
        $res=$focus->isRecordDeleteAllowed($module,$record);
        
        if(!$res)
        {
            $_SESSION['alert_msg']="You cant delete this record(s), this is linked with some other modules";
            return;
        }    
	...
	if ($module != $return_module && !empty($return_module) && !empty($return_id)) {
		$focus->unlinkRelationship($record, $return_module, $return_id);
            ...
            if($module == 'xFocusProductMapping')
            {
                $del_fd_chk_qry = $adb->pquery("select eff_from_date from vtiger_xfocusproductmapping where xfocusproductmappingid=?", array($record));
                $eff_from_date = $adb->query_result($del_fd_chk_qry, 0, 'eff_from_date');

                $del_bool = false;
                if($eff_from_date != '')
                {
                    $eff_from_date = strtotime($eff_from_date);
                    $cur_date = strtotime(date("Y-m-d"));

                    if($eff_from_date <= $cur_date)
                    {
                       $del_bool = true; 
                    }
                }
                if($del_bool)
                {
                    $_SESSION['alert_msg']="Selected record Effective From Date is passed, so you can't delete the record"; 
                    return;
                } 
            }
...
function DeleteEntity123($module,$return_module,$focus,$record,$return_id) { 
	global $log;	
	$log->debug("Entering DeleteEntity method ($module, $return_module, $record, $return_id)");
 ...
 * @author Kami @ Sify
 *
 * @param type star ttime
 * @param type end time
 * @param type working hours start time
 * @param type working hours end time
 * @param type weekly offs (array)
 * @param type Public Holidats (arry)
 * @return type Total SLA time 
 */

function getDuration($epoch_2,$epoch_1,$wh_stime,$wh_etime,$weekoffs,$pub_hol)
{
    $duration=0;
    
   //echo "<br/> Start Time:".date("Y-m-d H:i",$epoch_2);
   //echo "<br/> End Time:".date("Y-m-d H:i",$epoch_1);
    
    
     $taskStime=(date("H",$epoch_2)*60)+date("i",$epoch_2);
     $taskEtime=(date("H",$epoch_1)*60)+date("i",$epoch_1);
    
    //echo "<br/><br/> Task Start Time : ";
    //print_r($taskStime);

   // echo "<br/><br/> Task End Time : ";
    //print_r($taskEtime);
    
    //echo "<br/><br/> Work Start Time : ";
    //print_r($wh_stime);

    //echo "<br/><br/> Work End Time : ";
    //print_r($wh_etime);
    
    if($epoch_1>$epoch_2)
    {
        
        if(date("d",$epoch_1) > date("d",$epoch_2) || date("m",$epoch_1) > date("m",$epoch_2) || date("Y",$epoch_1) > date("Y",$epoch_2))
        {        

            $diff=get_dates_diffrence($epoch_1,$epoch_2);
            $mins=$diff['m'];$secs=$diff['s'];

           

           //echo "<br/><br/>Diffrece Minutes:".$mins;
           //echo "<br/>Diffrece Seconds:".$secs;

            $perdayMins=1440; // 24*60

            $epoch_2_x=$epoch_2+(24*60*60);
            $epoch_1_x=$epoch_1-(24*60*60);  

            $x=date("Y-m-d",$epoch_2_x);
            $y=date("Y-m-d",$epoch_1_x);
           ...
           //echo "<br/>";
            ...
            $alldays=get_days_n_dates_wdays($x,$y);

           //echo "<br/>All Days:";
           // print_r($alldays);

            $toRemoveDaysByPubHol=0;
            $toRemoveDaysByWeekOff=0;

            $workedDays;

            if($alldays[1]!=0)
                $workedDays=$alldays[1];

            foreach ($alldays[1] as $day=>$dayname)
            {
                //echo "<br/>".$dayname;
                //print_r($weekoffs);
                if(in_array($day, $pub_hol))
                {
                    $toRemoveDaysByPubHol++;
                    unset($workedDays[$day]);
                    continue;
                }      
                if(in_array($dayname,$weekoffs))
                {
                    $toRemoveDaysByWeekOff++;
                    unset($workedDays[$day]);
                    continue;
                }
            }

            //echo "<br/><br/> Total Week Offs :";
            //print_r($toRemoveDaysByWeekOff);

           // echo "<br/><br/> Total Pub Holdyas :";
            //print_r($toRemoveDaysByPubHol);

            $workingMinutes=$wh_etime-$wh_stime;

            $minutesToRemovePerDay=$perdayMins-$workingMinutes;

            //echo "<br/><br/> Total Minutes to remove on Working Days:";
            //print_r($minutesToRemovePerDay);

            $totalMinsToRemove=($toRemoveDaysByPubHol+$toRemoveDaysByWeekOff)*(24*60);

            //echo "<br/><br/> Total Minutes to remove those days not worked : ";
            //print_r($totalMinsToRemove);

            $diff_min=$mins-$totalMinsToRemove;

            //echo "<br/><br/> After Removing Non Working Days : ";
            //print_r($diff_min);

            //echo "<br/><br/> Total Working Days :";
            //print_r($workedDays);

            $diff_min_to_cons= count($workedDays)*$workingMinutes;

           // echo "<br/><br/> Total Minutes Worked : ";
            //print_r($diff_min_to_cons);

            $carryMins=0;

            if($taskStime < $wh_stime)
            {   
                $taskStime=$wh_stime;
            }
            
            if($taskStime < $wh_etime)
                 $carryMins+=$wh_etime-$taskStime;    
            
            if($taskEtime > $wh_stime)
            {
                if($taskEtime > $wh_etime)
                    $taskEtime=$wh_etime;

                 $carryMins+=$taskEtime-$wh_stime;    
            }   

            //echo "<br/><br/> Total Minutes Worked on ext days : ";
            //print_r($carryMins);

            //echo "<br/><br/> Total Time in Minutes : ";
            //print_r($carryMins+$diff_min_to_cons);
            
            $duration=$carryMins+$diff_min_to_cons;
            
        }
        else if(date("d",$epoch_1) == date("d",$epoch_2) && date("m",$epoch_1) == date("m",$epoch_2) && date("Y",$epoch_1) == date("Y",$epoch_2))
        {
            $carryMins=0;
            ...
            //echo "<br/>tst:".$taskStime;
            //echo "<br/>tet:".$taskEtime;
            
            $carryMins+=($taskEtime-$taskStime);    

            //echo "<br/><br/> Total Minutes Worked on the day : ";
            //print_r($carryMins);
 ...
 * 
 * @param type end time
 * @param type start time
 * @return type diffrence in minutes & Secs
 */

function get_dates_diffrence($epoch_1,$epoch_2)
{
    $return=array();
...
function generateCommaSeparatedString($arrayToDO,$quot)
{
    $toReturn="";
...
function getIndianCurrencyEq($amount)
{
    if($amount/10000000 >=1)
    {
        return  round(($amount/10000000),2)." Crores";
    }
    else if($amount/100000 >=1)
    {
        return round(($amount/100000),2)." Lakhs";
    }
    else if($amount/1000 >=1)
    {
        return round(($amount/1000),2)." Thousands";
    }
    else
    {
...
function getIndianCurrencyEqinLkhs($amount)
{
...
function getUSCurrencyEq($amount)
{

    if($amount/1000000000 >=1)
    {
        return round(($amount/1000000000),2)." Billion $";
    }
    else if($amount/1000000 >=1)
    {
        return round(($amount/1000000),2)." Million $";
    }
    else if($amount/1000 >=1)
    {
...
function getUSCurrencyEqForIndianAmount($amount)
{
    global $adb;
    
    $result=$adb->pquery("SELECT conversion_rate FROM vtiger_currency_info where currency_code='USD'");
    
    $conValue=$adb->query_result($result,0,'conversion_rate');
...
function getUSCurrencyEqforIndianAmountinThousands($amount)
{
    global $adb;
...
function getCurrentQuarterDetails()
{
    $today = new DateTime();
    $currentYear =(int)$today->format('Y');
    $currentmonth=(int)$today->format('m');
    $currQuarter = ceil(( $currentmonth + ( 21 -  ( 4 -1 ))) /3 ) %4 +1 ;
    $currQtr="Q".$currQuarter;
    if($currentmonth>=4)
    {
    $cy=$currentYear;
    $ny1=++$currentYear;
    ...
    $ny=substr($ny1, -2);
    $concat="-";
    $finyear=$cy.$concat.$ny;
    
    $yearsDate="01-04-".$cy;
    $yeareDate="31-03-".($cy+1);
    
    switch ($currQuarter)
    {
    case 1:
        $date1= "01-4-".(int)$today->format('Y');
        $lastdateofmonth=date('t',strtotime("6/1/".(int)$today->format('Y')));
        $date2=(int)$today->format('d')."-".(int)$today->format('m')."-".(int)$today->format('Y');
        $date3=(int)$lastdateofmonth."-06-".(int)$today->format('Y');
        break;
    case 2:
        $date1="01-7-".(int)$today->format('Y');
        $lastdateofmonth=date('t',strtotime("9/1/".(int)$today->format('Y')));
        $date2=(int)$today->format('d')."-".(int)$today->format('m')."-".(int)$today->format('Y');
        $date3=(int)$lastdateofmonth."-09-".(int)$today->format('Y');
          break;
    case 3:
        $date1= "01-10-".(int)$today->format('Y'); ;
        $lastdateofmonth=date('t',strtotime("12/1/".(int)$today->format('Y')));
        $date2=(int)$today->format('d')."-".(int)$today->format('m')."-".(int)$today->format('Y');
        $date3=(int)$lastdateofmonth."-12-".(int)$today->format('Y');
        break;
    default:
        $date1="01-1-".(int)$today->format('Y');
        $lastdateofmonth=date('t',strtotime("3/1/".(int)$today->format('Y')));
        $date2=(int)$today->format('d')."-".(int)$today->format('m')."-".(int)$today->format('Y');
        $date3=(int)$lastdateofmonth."-03-".(int)$today->format('Y');
      } 
      
      
    $toReturn['sDate']=$date1;
    $toReturn['cDate']=$date2;
    $toReturn['eDate']=$date3;
    $toReturn['yearsDate']=$yearsDate;
    $toReturn['yeareDate']=$yeareDate;
    
    return $toReturn;
    
    }
    
    function getFromAndEndDateofQuarter($year,$qtr)
    {
        $toReturn;
        ...
        if($qtr!='')
            {
                ...
                if($month!='')
                     {
                        $toReturn['from']="01-".$month."-".$ys;
                        $toReturn['to']="31-".($month+2)."-".$ys;                                                  
                    }
            }  
     ...
     */    
    function getQuarterDetailsByUsingDate($date)
    {
        $ytoSend;$qtoSend;
    ...
    function checkPotentialType($potRenArray,$potAcctArray,$poefdecRes,$key)
    {        
        //print_r($potRenArray);   
        //echo $key.":".$potRenArray[$key]."<br/>";
        
//        if($potRenArray[$key]=='1')
//            return "Renewed";
        
        $keyAct=$potAcctArray[$key];
        
        $accts=array_values($poefdecRes);        
        $acctCount=array_count_values($accts);
        
        $actTimes=$acctCount[$keyAct];
        
//        echo $key;
//        if($key=="kotak securities")
//        {
//            echo "KA:".$keyAct;
//            echo "accts:".print_r($accts);
//            echo "acctCount:".print_r($acctCount);
//            echo "actTimes:".print_r($actTimes);
//        }
    ...
    $durationBLk['Per Year']=1;
    $durationBLk['Per Quarter']=4;
    $durationBLk['Per Month']=12;
    
    function splitAmount($c1,$c2,$c3,$c4,$c5,$c6,$c7,$c8,$prob,$ren,$currency){
        $oa='-';$ra='-';
        global $durationBLk;
        ...
        {                        
            ...
            {
//                 if($currency=="Rs")
//                    $oa=getIndianCurrencyEqinLkhs($c1);
//                 else
//                    $oa=getUSCurrencyEqforIndianAmountinThousands($c1);
                  $oa=$c1;
                 
                 
                 $temp=$c2*$durationBLk[$c4]*$c3;
                 
//                 if($currency=="Rs")
//                    $ra=getIndianCurrencyEqinLkhs($temp);
//                 else
//                    $ra=getUSCurrencyEqforIndianAmountinThousands($temp);
                 $ra=$temp;
            }
            else
            {
    ...
    function getCurrenyForTheReports($curr,$amount)
    {
        if($curr!='$')
            return getIndianCurrencyEq ($amount);
        else
            return getUSCurrencyEq(getUSCurrencyEqforIndianAmountinThousands($amount)*1000);
    }
    
    function getCurrenyForTheReportsWithoutText($curr,$amount)
    {
        if($curr!='$')
            return getIndianCurrencyEqinLkhs($amount);
        else
            return getUSCurrencyEqforIndianAmountinThousands ($amount);
    }
    
    /**
 * @author Shaik Khamuruddeen <shaik.khamuruddeen@sifycorp.com>
 * @param String $keyName : Password Policy
 * @param String $value   : Value of the Policy
 * @param String $userId  : User Id
 * @param String $newPwd  : Encrypted New Password
 * @param String $newPassword : Unencrypted New Pasword
 * @param String $userName : User Name of the User
 * @return array :  (key=>value) <br/>
 *  <b>key</b> : true/false <br/>
 *  <b>value</b> : description , ERROR message if key is false
 */

function passFormula($keyName,$value,$userId,$newPwd,$newPassword,$userName)
{
    global $adb;
    switch ($keyName) {
        case 'pwd_reuse_count':
            
            if(empty($userId))
                return array(true=>'Not Applicable..!');
            
            $last3Pwds=$adb->pquery("SELECT sify_xuserpassword_history.password FROM sify_xuserpassword_history where sify_xuserpassword_history.userid=? LIMIT 0,$value",array($userId));
            for ($index = 0; $index < count($last3Pwds); $index++) {
                $oldPwd=$adb->query_result($last3Pwds,$index,0);

                if($newPwd==$oldPwd)
                    return array(false=>'Repeated password is not allowed .!');
            }
        ...
        case 'pwd_invalid_charset':

            $naArray=split(',', $value);
            $testArray=str_split($newPassword);
            //$testArray=  array_keys($testArray);

            for ($index1 = 0; $index1 < count($naArray); $index1++) {
                $splChar=$naArray[$index1];
                $splChar= trim($splChar);

                $testX=in_array($splChar,$testArray);

                 if($testX)
                    return array("false"=>"Special Character({$splChar}) is not allowed") ;
            }
       ...
       case 'pwd_min_length':
           ...
           if(strlen(trim($newPassword))<$value)
           {
               return array("false"=>"Password Min Length : $value") ;
           }
       ...
       case 'pwd_max_length':
           ...
           if(strlen(trim($newPassword))>$value)
           {
               return array("false"=>"Password Max Length : $value") ;
           }
        ...
        case 'pwd_dup_splchar_allowed':

           if($value=='1')
               return array(true=>true);
           else
           {
               $naArray=str_split($newPassword);
               $duplicates=getDuplicatesFromArray($naArray);
               
               foreach ($duplicates as $key => $value) {
                    if(typeOfChar($value)=='splchar')
                       return array("false"=>"Duplicate Special Charecters are not allowed");
               }
                return array(true=>true);
           } 
           break;
        
       case 'pwd_dup_numchar_allowed':
           ...
           else
           {
               ...
               $duplicates=getDuplicatesFromArray($naArray);
                ...
                foreach ($duplicates as $key => $value) {
                    if(typeOfChar($value)=='number')
                       return array("false"=>"Duplicate numbers are not allowed");
               }
            return array(true=>true);
           } 
           break;
        
       case 'pwd_dup_alpha_allowed':
           ...
           else
           {
               ...
               $duplicates=getDuplicatesFromArray($naArray);
                ...
                foreach ($duplicates as $key => $value) {
                    if(typeOfChar($value)=='captalpha' || typeOfChar($value)=='smallalpha' )
                       return array("false"=>"Duplicate Alphabetics are not allowed");
               }
            return array(true=>true);
           } 
           break;
        
       case 'pwd_userid_same':
           ...
           if($userName==$newPassword)
               return array("false"=>"Password Can not be same as user name");
               
               
            return array(true=>true);
           break;
        
       case 'pwd_min_splchar':
       case 'pwd_min_numchar':
       case 'pwd_min_alpha':
           
           if($value<=0)
               return array(true=>true);
           
           $result=getTheTypesOfCharInString($newPassword);
           
         switch ($keyName) {
            case 'pwd_min_splchar':
                
                if($result['splchar']<$value)
                    return array("false"=>"Password has to contain minimum $value special charecters");
            ...
            case 'pwd_min_numchar':
                ...
                if($result['num']<$value)
                    return array("false"=>"Password has to contain minimum $value numbers");
            ...
            case 'pwd_min_alpha':
                ...
                if($result['char']<$value)
                    return array("false"=>"Password has to contain minimum $value alphabets");
...
function getDuplicatesFromArray($tempArray)
{
    $unique= array_unique($tempArray);
    $dup=array_diff_assoc($tempArray, $unique);
    return $dup;    
}
 ...
 * @return string :
 *      <br/> 'number' : if character is a number
 *      <br/> 'captalpha' : if character is a Captial Aplhabet
 *      <br/> 'smallalpha' : if character is a Small Aplhabet
 *      <br/> 'splchar' : if character is a special charecter
 *  
 */
...
function typeOfChar($char)
{
    ...
    switch (true) {
        case ($asChar>=48 && $asChar<=57):
            return 'number';
            break;
        case ($asChar>=65 && $asChar<=90):
            return 'captalpha';
            break;
        case ($asChar>=97 && $asChar<=122):
            return 'smallalpha';
            break;
        default:
            return 'splchar';
...
/**
 * @author Shaik Khamuruddeen <shaik.khamuruddeen@sifycorp.com>
 * @param String $data : Input string to search
 ...
 *   <br/>
 * 
 *   Returns the no of characters , numbers & special characters in the string 
 *   
 */

function getTheTypesOfCharInString($data)
{
   $ch=0;$num=0;$splch=0;
    
    
   $charArray= str_split($data);
   foreach ($charArray as $key => $value) {
       $temp=typeOfChar($value);
       if($temp=='number')
           $num++;
       else if($temp=='captalpha' || $temp=='smallalpha')
           $ch++;
       else if($temp=='splchar')
           $splch++;
   }
   
   return array('char'=>$ch,'num'=>$num,'splchar'=>$splch);
}
    
    
    /**
     *
     * @param array $map - Array to convert as XML
     * @param String $filepath - Filepath to save XML
     * @param String $filename - File Name 
     * @param String $xmlRootName  - Root tag name of the XMl Ex: <?xml version=\"1.0\"?><collections></collections>
     */
    
    function convert_array_to_xml($map,$filepath,$filename,$xmlRootName,$repeatTags=array())
    {
        $xml_student_info = new SimpleXMLElement("<?xml version=\"1.0\"?><".$xmlRootName."></".$xmlRootName.">");
        
        //print_r($map);
        
        array_to_xml($map,$xml_student_info);
        
        if(!is_dir($filepath))
            mkdir($filepath); 
        
        //$xml_student_info->asXML($filepath.'/'.$filename);
        
        //print_r($repeatTags);
        
        foreach ($repeatTags as $repeatTagskey => $repeatTagsvalue) {
            
            $repeatTagsvalue=str_replace(":::","/", $repeatTagsvalue);
            
            $xpRes=$xml_student_info->xpath('/'.$xmlRootName.'/'.$repeatTagsvalue);
             //echo '/'.$xmlRootName.'/'.$repeatTagsvalue;
             //print_r($xpRes);
            
            while(list( , $node) = each($xpRes)) {
                //print_r($node);
                $node->addAttribute('repetitiveMapTag','/'.$xmlRootName.'/'.$repeatTagsvalue);
             }
        }
        
        $xml_student_info->saveXML();
        //exit;
        $xml_student_info->asXML($filepath.'/'.$filename);
    }
    ...
    function array_to_xml($student_info, &$xml_student_info) {
        foreach($student_info as $key => $value) {
            if(is_array($value)) {
                ...
                $attPosition=strpos($key,'@');
                //echo "$key".'---'.$attPosition;
                if($attPosition!==FALSE)
                {
                    //echo "@@@".$key;
                    $keyX=substr($key,$attPosition+1);
                    //echo 'kami'.$keyX;
                    $xml_student_info->addAttribute("$keyX","$value");
                }
                else 
                {
...
//                $xml_student_info->addChild("$key","$value");
    ...
    //modify entity To restrict edit option if the values related to some other master
    
  function EditEntityval($module,$return_module,$focus,$record,$return_id) {
	global $log,$adb;	
	$log->debug("Entering ModityEntity method ($module, $return_module, $record, $return_id)");
	
        $focus = CRMEntity::getInstance($module);
        //$res=$focus->isRecordDeleteAllowed($module,$record);
        $resData = array();
        if($record!=""){ 
         $linkResults=$adb->pquery("SELECT vtiger_field.columnname,vtiger_field.tablename,vtiger_field.tabid FROM vtiger_fieldmodulerel
                                        inner join vtiger_field on vtiger_field.fieldid=vtiger_fieldmodulerel.fieldid 
                                        where relmodule=?",array($module));
          
            for ($index = 0; $index < $adb->num_rows($linkResults); $index++) {
                $tabName=$adb->query_result($linkResults,$index,'tablename');
                $colName=$adb->query_result($linkResults,$index,'columnname');
                $coltabId=$adb->query_result($linkResults,$index,'tabid');
                
                $colRes=$adb->pquery("SELECT entityidfield FROM vtiger_entityname where tabid=?",array($coltabId));
                $entCol=$adb->query_result($colRes,0,'entityidfield');
                
                
                $recResults=$adb->pquery("SELECT count(*) as count FROM {$tabName} 
                                          LEFT JOIN vtiger_crmentity on vtiger_crmentity.crmid={$tabName}.{$entCol} where {$colName}=? and vtiger_crmentity.deleted=0",array($record));
                $resData[]=$adb->query_result($recResults,0,'count');
	...
	$log->debug("Exiting EditEntity method ...");
}  

function getLaguageDisplayName($language,$key)
{
    if($language[$key]!='')
        return $language[$key];
    else
...
function configimages($imagename,$configchk){
   //echo"hii"; print_R($imagename);die;
global $config_image_path;
if(!empty($imagename) && !empty($config_image_path))
{
   if($configchk !=0 && file_exists($config_image_path.$imagename)){
     $returnpath = $config_image_path.$imagename;
   }
   else
   {
     $returnpath="";   
 ...
 //print_R($config_image_path.$imagename);
//print_R(file_exists($config_image_path.$imagename));die;
return $returnpath;
}
