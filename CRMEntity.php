...
require_once 'modules/PickList/PickListUtils.php';
 ...
 $cus_host=$dbconfig['db_server'];
 $cus_user=$dbconfig['db_username'];
 $cus_pass=$dbconfig['db_password'];
 $cus_port=$dbconfig['db_port'];
 $cus_db=$dbconfig['db_name'];
...
class CRMEntity
{
	...
	function saveentity($module,$fileid='')
	{
		...
		$massEditSaveActionCheck=vtlib_purify($_REQUEST['action']);
                
                
		if(PerformancePrefs::getBoolean('NOTIFY_OWNER_EMAILS', true) == 1){                    
			if($massEditSaveActionCheck!='MassEditSave' && ( 1==1 || ($module != "Calendar" && $module != "Accounts" && $module != "Contacts") ) ){                            
			...
			if($on_focus->IsCustomModule && method_exists($on_focus, 'save_related_module')) {
				...
				$on_focus->save_related_module(
					$for_module, $for_crmid, $with_module, $with_crmid);
                                ...
                                $adb->pquery("CALL prc_createat_updateat_data(?,?,?)", array($for_module,$for_crmid,2));//1 -- DELETE   0 -- INSERT   2 -- UPDATE
			}
		}
                /*PROCEDURE FOR RECORD CREATION, UPDATION AND DELETE OPTION MAINTAIN EVERY TABLE*/
                if($insertion_mode=='edit'){
                    $adb->pquery("CALL prc_createat_updateat_data(?,?,?)", array($module,$this->id,2));//1 -- DELETE   0 -- INSERT   2 -- UPDATE
                }else{
                    $adb->pquery("CALL prc_createat_updateat_data(?,?,?)", array($module,$this->id,0));//1 -- DELETE   0 -- INSERT   2 -- UPDATE
                }
                /* PROCEDURE END */
	// END
	//echo "1.2";
	...
	function insertIntoAttachment1($id,$module,$filedata,$filename,$filesize,$filetype,$user_id)
	{
		...
		$adb->pquery($sql, $params);
                $setype_id = getTabid($module);

		$sql1 = "insert into vtiger_crmentity (crmid,smcreatorid,smownerid,setype,setype_id,description,createdtime,modifiedtime) values(?, ?, ?, ?, ?, ?, ?, ?)";
		$params1 = array($current_id, $current_user->id, $ownerid, $module." Attachment", $setype_id, '', $adb->formatDate($date_var, true), $adb->formatDate($date_var, true));
	...
	*/
	...
	{		
		...
		if($module == 'Contacts' || $module == 'Products' || $module == 'Accounts' || $module == 'Company')
		{
		...
		if($save_file == 'true' && $upload_status == 'true')
		{
			...
			if($module == 'Contacts' || $module == 'Products' || $module == 'Accounts')
			{
				$sql1 = "insert into vtiger_crmentity (crmid,smcreatorid,smownerid,setype,setype_id,description,createdtime,modifiedtime) values(?, ?, ?, ?, ?, ?, ?, ?)";
				$params1 = array($current_id, $current_user->id, $ownerid, $module." Image", $setype_id, $this->column_fields['description'], $adb->formatDate($date_var, true), $adb->formatDate($date_var, true));		
			}
			else
			{
				$sql1 = "insert into vtiger_crmentity (crmid,smcreatorid,smownerid,setype,setype_id,description,createdtime,modifiedtime) values(?, ?, ?, ?, ?, ?, ?, ?)";
				$params1 = array($current_id, $current_user->id, $ownerid, $module." Attachment", $setype_id, $this->column_fields['description'], $adb->formatDate($date_var, true), $adb->formatDate($date_var, true));		
			}
			$adb->pquery($sql1, $params1);
			...
			if($module == 'Contacts')
			{
				$att_sql="select vtiger_seattachmentsrel.attachmentsid  from vtiger_seattachmentsrel inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_seattachmentsrel.attachmentsid where vtiger_crmentity.setype_id=? and vtiger_seattachmentsrel.crmid=?";
				$res=$adb->pquery($att_sql, array($setype_id, $id));
  ...
  function insertIntoCrmEntity($module,$fileid='')
  {
	...
	if($this->mode == 'edit')
	{
		...
		$tabid = getTabid($module);
                    ...
                    $dbquy = 'select smownerid,modifiedby,smcreatorid from vtiger_crmentity where crmid = ?';
                    $reslt = $adb->pquery($dbquy, array($this->id));
                   $prevownerid = $adb->query_result($reslt,0,'smownerid');
	...
	else
	{
		...
		$setype_id = getTabid($module);
		$description_val = from_html($this->column_fields['description'],($insertion_mode == 'edit')?true:false);
		$sql = "insert into vtiger_crmentity (crmid,smcreatorid,smownerid,setype,setype_id,description,createdtime,modifiedtime) values(?,?,?,?,?,?,?,?)";
		$params = array($current_id, $current_user->id, $ownerid, $module, $setype_id, $description_val, $adb->formatDate($date_var, true), $adb->formatDate($date_var, true));
        ...
        {
             $asgndsql = "insert into sify_assignedtohistory(crmid,assignedto,createdtime,assignedfrom) values(?,?,?,?)";		
            $asgndparams = array($this->id,$ownerid,$adb->formatDate($date_var, true),$current_user->id);
            $adb->pquery($asgndsql,$asgndparams);
        }
   }
  ...
  function insertIntoEntityTable($table_name, $module, $fileid='')
  {
	...
	$tabid= getTabid($module);
	  ...
	  for($i=0; $i<$noofrows; $i++) {
		...
		if($uitype == 4 && $insertion_mode != 'edit') {
		  ...
		  if(isset($this->column_fields[$fieldname]))
		  {
			  ...
			  elseif($uitype == 15 || $uitype == 16)
			  {
				  ...
				  if($this->column_fields[$fieldname] == $app_strings['LBL_NOT_ACCESSIBLE'])
				  {
					...
					//If the value in the request is Not Accessible for a picklist, the existing value will be replaced instead of Not Accessible value.
					 ...
					 $res = $adb->mquery($sql,array($this->id));
				  ...
				  {
					  $fldvalue 		= $this->column_fields[$fieldname];
					  if($uitype == 15 && !empty($fldvalue)){
						  $roleid			= $current_user->roleid;
						  $picklistValues 	= getAssignedPicklistValues($fieldname, $roleid, $adb);
						  $Key 				= array_search(strtolower($fldvalue), array_map('strtolower', $picklistValues));
						  $fldvalue   		= $picklistValues[$Key];
						  if(empty($fldvalue)){
							 $fldvalue 		= $this->column_fields[$fieldname]; 
						  }
					  }
			  ...
			  elseif($uitype == 28){
				...
				$fldvalue = convertToDollar($this->column_fields[$fieldname], $curSymCrate['rate']);
			}elseif($uitype == 1032 && $insertion_mode != 'edit' && isset($_REQUEST['autogencode']) && $_REQUEST['autogencode']==1) {
                            $this->column_fields[$fieldname] = $this->seModSeqNumber("increment",$module);
                            $fldvalue                        = $this->column_fields[$fieldname];
                        }elseif($uitype == 1032 && $insertion_mode != 'edit' && !isset($_REQUEST['autogencode'])) {
                            $DuplicateCode                   = $this->duplicateCodeCheck($this->column_fields[$fieldname],$module,$table_name,$fieldname,$this->tab_name_index[$table_name]);
                            if($DuplicateCode==1){
                                $this->column_fields[$fieldname] = $this->seModSeqNumber("increment",$module);
                                $fldvalue                        = $this->column_fields[$fieldname];
                            }else{
                            ...
                            $RetriveData                     = $this->retrieve_autocode_info($this->id, $module,$table_name,$fieldname,$this->tab_name_index[$table_name]);
                            
                            if($RetriveData != $this->column_fields[$fieldname]){
                                $DuplicateCode                   = $this->duplicateCodeCheck($this->column_fields[$fieldname],$module,$table_name,$fieldname,$this->tab_name_index[$table_name]);
                                if($DuplicateCode==1){
                                    $this->column_fields[$fieldname] = $RetriveData;
                                    $fldvalue                        = $this->column_fields[$fieldname];
                                }else{
                                    $fldvalue                        = $this->column_fields[$fieldname];
                                }
	  ...
	  if($insertion_mode == 'edit')
	  {
		  ...
		  elseif($_REQUEST['module'] == 'PurchaseOrder' || $_REQUEST['module'] == 'SalesOrder' || $_REQUEST['module'] == 'Quotes' || $_REQUEST['module'] == 'SalesInvoice')
		  {
		  ...
		  if(count($update) > 0) {
			...
			$adb->pquery($sql1, $update_params);
                       ...
                       /*  $archive = $this->getModuleLevelArchive($module);
                        if($archive==0)
                            $adb->pquery("CALL prc_archive_table_log(?,?,?,?)", array($this->id,$table_name,$this->tab_name_index[$table_name], $module)); */
		  }
	  ...
	  else
	  {
		...
		$result = $adb->pquery($sql1, $value);
			if($adb->getAffectedRowCount($result)<=0 && $_REQUEST['module'] == 'SalesInvoice'){
                      $log->debug('getAffectedRowCount of table_name: '.$table_name);
...
function getModuleLevelArchive($module) {
    global $adb;
    $query   = "SELECT archive FROM `vtiger_tab` WHERE name=?  LIMIT 1";
    $result  = $adb->pquery($query,array($module));
    $archive = $adb->query_result($result,0,'archive');
    return $archive;
}
function whomToSendMail($module,$insertion_mode,$assigntype)
{
 	...
 	global $current_user;
   	...
   	if($insertion_mode!="edit")
   	{
              if($current_user->id == $this->column_fields['assigned_user_id'])
                return;
            
                    if($assigntype=='U' || $assigntype=='S')
                    {
            ...
            elseif($assigntype=='T')
            {
                            ...
                            if($module=="HelpDesk")
                                    sendNotificationToGroupsforHelpDesk($groupid,$this,$module); // By Kami
                            else
...
// Code included by Jaguar - Ends 
  ...
  function retrieve_entity_info($record, $module)
  {
    ...
    {
            $result[$table_name] = $adb->mquery("select * from $table_name where $index=?", array($record));
	...
	if (isset($this->table_name)) {
            ...
            //echo "Hi :".print_r($record);exit;
            if($adb->query_result($result[$this->table_name],0,$mod_index_col) == '')
            {
                /* FRPRDIN-292 */
                if($_REQUEST['destination_module']!='' && $_REQUEST['destination_module'] != 'xAddress'  && ($this->table_name != 'vtiger_xcdkeydistmapping' && $this->table_name != 'vtiger_xcdkey'))
                {
	...
	if($cachedModuleFields) {
        ...
        //echo "Hi :".$_REQUEST['module'].", ".$_REQUEST['destination_module'];exit;
        if($module == 'Users')
	{
		for($i=0; $i<$noofrows; $i++)
		{
	...
	function save($module_name,$fileid='') 
	{
		...
		$this->saveentity($module_name,$fileid);
           if(isset($this->column_fields['related_to']) && $this->column_fields['related_to']!='')
		   {			   
				$adb->pquery("UPDATE `vtiger_crmentity` SET `modifiedtime`=NOW() where `crmid`=?",array($this->column_fields['related_to']));
		   }
                
                //Added for Updating related table detailview record modified time also when updating or adding a subpanel records
                $returnId = $_REQUEST['return_id'];
                $return_module = $_REQUEST['return_module'];
                if(!empty($returnId) && isset($returnId) && !empty($return_module) && isset($return_module)){
                    $this->updateCrmModifiedDate($returnId,$return_module);
                }
	...
	*/
	...
	function trash($module, $id) {
		...
		$sql_recentviewed ='DELETE FROM vtiger_tracker WHERE user_id = ? AND item_id = ?';
                ...
                /*Delete Updation for All Tables*/
                $this->db->pquery("CALL prc_createat_updateat_data(?,?,?)", array($module,$id,1));//1 -- DELETE   0 -- INSERT   2 -- UPDATE
                /* END */
	}
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		for ($i=0; $i<$numOfFields; $i++) {
			...
			$this->db->pquery($updateQuery, $updateParams);			
                  ...
                  $this->db->pquery("UPDATE `vtiger_crmentity` SET `modifiedtime`=NOW() where `crmid`=?",array($return_id));
	}
		...
		for($i=0; $i < $numRows;$i++)
		{
			...
			if(strtoupper($action) == RB_RECORD_UPDATED) {
				...
				if ($rel_table == 'vtiger_seproductrel') {
					$sql = "INSERT INTO $rel_table($rel_column, $ref_column, 'setype', 'setype_id') VALUES (?,?,?,?)";
                                        $setype_id = getTabid($module);
					$this->db->pquery($sql, array($record, $related_crm_ids, $module, $setype_id));
				} else {
	...
	*/
	...
	function setModuleSeqNumber($mode, $module, $req_str='', $req_no='')
	{
		...
		if ($mode == "configure" && $req_no != '') {
			...
			//sem_release();*/
//			global $cus_host,$cus_user,$cus_pass,$cus_port,$cus_db;
//			$con = mysql_connect($cus_host,$cus_user,$cus_pass,false,$cus_port);
// 			mysql_select_db($cus_db,$con);
                        // mysql_query("SELECT GET_LOCK('sifycrm.inclick',10)"); commented by senthil on 22 Dec 2013
                        $adb->mquery("CALL increment_id('".$module."',@ret)");
                        $res = $adb->mquery("SELECT @ret");
                        
                        $row = $adb->query_result($res,0,0);
			return $row;	
		}
	...
	/* Function to set the Level Sequence string and sequence number starting value */
	function seModSeqNumber($mode, $module, $req_str = '', $req_no = '') {
            global $adb;
            ...
            if ($mode == "increment") {
                ...
                //$query = "SELECT count(*) as cnt FROM vtiger_".strtolower($module);
                $setype_id = getTabid($module);
                $query      = "SELECT count(*) as `cnt` FROM vtiger_crmentity where setype_id=?";
                $idleResult = $adb->mquery($query, array($setype_id));
                $count      = $adb->query_result($idleResult, 0, 'cnt');
                $row        = $count + 1;
                $Code       = $this->generateAutoCode($prefix,$row,$module);
        ...
        function generateAutoCode($prefix,$row,$module) {
            global $adb,$root_directory;
            
            $filePath   = $root_directory."/storage/log/autocodegen/";
            $fileName   = $filePath."/autocodegen_result_".date("Y_m_d").".txt";
            if (!file_exists($filePath)) {       
                mkdir($filePath, 0777, true);
            }
            $fp         = fopen($fileName, 'r+');
            $resulst = "+++++++ - Log Time - ".date("Y-m-d H:i:s")." +++++++".PHP_EOL;
            ...
            if(!empty($AutoCode)){
                ...
                $FQuery      = "SELECT columnname,tablename FROM vtiger_field WHERE tabid=? AND uitype IN (1032) AND presence IN(0,2)";
                $FResult     = $adb->mquery($FQuery, array($tabid));
                if($adb->num_rows($FResult)>0){
                    $COLUMNNAME  = $adb->query_result($FResult, 0, 'columnname');
                    $TABLENAME   = $adb->query_result($FResult, 0, 'tablename');


                    file_put_contents($fileName, $FQuery.PHP_EOL , FILE_APPEND | LOCK_EX);
                    file_put_contents($fileName, $COLUMNNAME." ".$TABLENAME.PHP_EOL , FILE_APPEND | LOCK_EX);

                    if(!empty($COLUMNNAME) && !empty($TABLENAME)){
                        $DupQuery       = "SELECT MAX(CAST(RIGHT($COLUMNNAME, LENGTH($COLUMNNAME)-(myPos-1)) as SIGNED)) AS dupcount FROM (SELECT 
                                            $COLUMNNAME, 
                                            LEAST (
                                              if (Locate('0',$COLUMNNAME) >0,Locate('0',$COLUMNNAME),999),
                                              if (Locate('1',$COLUMNNAME) >0,Locate('1',$COLUMNNAME),999),
                                              if (Locate('2',$COLUMNNAME) >0,Locate('2',$COLUMNNAME),999),
                                              if (Locate('3',$COLUMNNAME) >0,Locate('3',$COLUMNNAME),999),
                                              if (Locate('4',$COLUMNNAME) >0,Locate('4',$COLUMNNAME),999),
                                              if (Locate('5',$COLUMNNAME) >0,Locate('5',$COLUMNNAME),999),
                                              if (Locate('6',$COLUMNNAME) >0,Locate('6',$COLUMNNAME),999),
                                              if (Locate('7',$COLUMNNAME) >0,Locate('7',$COLUMNNAME),999),
                                              if (Locate('8',$COLUMNNAME) >0,Locate('8',$COLUMNNAME),999),
                                              if (Locate('9',$COLUMNNAME) >0,Locate('9',$COLUMNNAME),999)
                                            ) as myPos
                                          FROM $TABLENAME) AS NUMFIND ";

                        file_put_contents($fileName, $DupQuery.PHP_EOL , FILE_APPEND | LOCK_EX);

                        $DupResult      = $adb->mquery($DupQuery, array());
                        $DUPLICATECOUNT = $adb->query_result($DupResult, 0, 'dupcount');

                        if($DUPLICATECOUNT>0){
                            $AutoCode = $prefix.($DUPLICATECOUNT+1);
                        }
                        file_put_contents($fileName, $AutoCode.PHP_EOL , FILE_APPEND | LOCK_EX);
                    }
                }
                return $AutoCode;
            }
        ...
        function retrieve_autocode_info($AutoCodeID,$module,$TABLENAME = '',$COLUMNNAME = '',$TableID) {
            global $adb;
            if(!empty($AutoCodeID)){
                if(empty($TABLENAME) && empty($COLUMNNAME)){
                    $tabid       = getTabid($module);
                    $FQuery      = "SELECT columnname,tablename FROM vtiger_field WHERE tabid=? AND uitype=1032 AND presence IN(0,2)";
                    $FResult     = $adb->mquery($FQuery, array($tabid));
                    $COLUMNNAME  = $adb->query_result($FResult, 0, 'columnname');
                    $TABLENAME   = $adb->query_result($FResult, 0, 'tablename');
                }
                $DupQuery       = "SELECT $COLUMNNAME AS code FROM $TABLENAME WHERE deleted=0 AND $TableID=?";
                $DupResult      = $adb->mquery($DupQuery, array($AutoCodeID));
                $CODERESULT     = $adb->query_result($DupResult, 0, 'code');
        ...
        function duplicateCodeCheck($AutoCode,$module,$TABLENAME = '',$COLUMNNAME = '',$TableID) {
            global $adb;
            if(!empty($AutoCode)){
                if(empty($TABLENAME) && empty($COLUMNNAME)){
                    $tabid       = getTabid($module);
                ...
                $DupQuery       = "SELECT count($TableID) AS dupcount FROM $TABLENAME WHERE deleted=0 AND  $COLUMNNAME=?";
                $DupResult      = $adb->mquery($DupQuery, array($AutoCode));
                $DUPLICATECOUNT = $adb->query_result($DupResult, 0, 'dupcount');
                if($DUPLICATECOUNT>0)
                    $AutoCode = 1;
                else
    ...
    function setLevelSeqNumber($mode, $module, $req_str='', $req_no='')
	{
            //$pa = "modules/".$module."/".$module.".php";
            //require($pa);
            //print $pa;
...
//                $query = "SELECT count(*) as cnt FROM vtiger_".strtolower($module);
                $query = "SELECT MAX(cf_".strtolower($module)."_hierarchy_level) lev_max FROM vtiger_".strtolower($module)."
                INNER JOIN vtiger_".strtolower($module)."cf ON vtiger_".strtolower($module)."cf.".strtolower($module)."id=vtiger_".strtolower($module).".".strtolower($module)."id
                INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_".strtolower($module).".".strtolower($module)."id
                WHERE vtiger_crmentity.deleted=0";


                $idleResult = $adb->mquery($query,array());
                $count = $adb->query_result($idleResult,0,'lev_max');
                $row = $count+1;
		return $row;	
            }	
	}
	// END
        ...
        function getModuleSeqInfoByMask($module,$prefix,$idMask) {
		global $adb;
		$check = $adb->mquery("select cur_id,prefix from vtiger_modentity_num where semodule=? and active = 1", array($module));
		$prefix = $adb->query_result($check,0,'prefix');
		$curid = $adb->query_result($check,0,'cur_id');
	...
	function checkModuleSeqNumber($table, $column, $no)
	{
		global $adb;
                ...
                return false;//THIS RETURN ADDED FOR SHOWING ERROR MESSAGE ON CREATE VIEW - VTIGER CODE DUPLIACTION WE ARE NOT USING
			...
			if($with_module == 'Documents') {	
				...
				$adb->pquery("INSERT INTO vtiger_crmentityrel(crmid, module, relcrmid, relmodule) VALUES(?,?,?,?)", 
					Array($crmid, $module, $relcrmid, $with_module));
                                ...
                                $setype_id = getTabid($module);
				$adb->pquery("UPDATE vtiger_crmentity SET modifiedtime = ? WHERE setype_id = ? AND crmid = ?", 
					Array(date('Y-m-d H:i:s', time()), $setype_id, $crmid));
			}
		}
	}
		...
		foreach($with_crmid as $relcrmid) {
			...
			if($with_module == 'Documents') {
				...
				$adb->pquery("DELETE FROM vtiger_crmentityrel WHERE crmid=? AND module=? AND relcrmid=? AND relmodule=?",
					Array($crmid, $module, $relcrmid, $with_module));	
                                $this->updateCrmModifiedDate($crmid, $module);
			}
		}
	}
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query .= " INNER JOIN ".$other->table_name."cf ON vtiger_crmentity.crmid = ".$other->table_name."cf.$other->table_index";
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($numOfFields > 0) {		
			...
			$dependentField = $this->db->query_result($dependentFieldSql, 0, 'fieldname');
                         if($currentModule=='Project' && $related_module=='ProjectMilestone' ){
                        $result=$this->db->pquery("select vtiger_project.linktoaccountscontacts from vtiger_project where projectid=?",array($id));
                         $relatedtoid=$this->db->query_result($result,0,'linktoaccountscontacts');
                        $button .= '<input type="hidden" name="cf_projectmilestone_account" id="cf_projectmilestone_account" value="'.$relatedtoid.'">';			
			$button .= '<input type="hidden" name="cf_projectmilestone_account_type" id="cf_projectmilestone_account_type" value="'.$currentModule.'">';
                         }
				...
				if(in_array('ADD', $actions) && isPermitted($related_module,1, '') == 'yes' 
					...
					$button .= "<input title='".getTranslatedString('LBL_ADD_NEW'). " ". getTranslatedString($singular_modname, $related_module) ."' class='crmbutton small create'" .
						...
						" value='". getTranslatedString('LBL_ADD_NEW'). " " . getTranslatedString($singular_modname, $related_module) ."'>&nbsp;";
                            ...
                            if($currentModule=='Project' && $related_module=='ProjectMilestone' )
                            {
                            $query = "SELECT vtiger_crmentity.*, $other->table_name.*,vtiger_projectmilestonecf.*";
                            }
                            else
                            {
			...
			$query .= " INNER  JOIN $this->table_name   ON $this->table_name.$this->table_index = $other->table_name.$dependentColumn";
                    if($currentModule=='Project' && $related_module=='ProjectMilestone' )
                    {
                    $query .= " INNER JOIN vtiger_projectmilestonecf ON vtiger_projectmilestone.projectmilestoneid=vtiger_projectmilestonecf.projectmilestoneid";
                    }
			$query .= $more_relation;
			$query .= " LEFT  JOIN vtiger_users        ON vtiger_users.id = vtiger_crmentity.smownerid";
			$query .= " LEFT  JOIN vtiger_groups       ON vtiger_groups.groupid = vtiger_crmentity.smownerid";
	...
	function add_related_to($module, $fieldname){
		...
		$query = "SELECT vtiger_crmentity.deleted, ".$entityNameArr['tablename'].".* 
					FROM ".$entityNameArr['tablename']."
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=".$entityNameArr['tablename'].".$focus1->table_index
	...
	function setRelationTables($secmodule){
        ...
        function isRecordDeleteAllowed($moduleName,$record)
        {
            global $adb;
            ...
            $linkResults=$adb->pquery("SELECT vtiger_field.columnname,vtiger_field.tablename,vtiger_field.tabid FROM vtiger_fieldmodulerel
                                        ...
                                        where relmodule=?",array($moduleName));
        ...
        /**
         * Gives Query for new pop-up combo 
         * 
         * @param type $modueName
         */
        
        function getComboPopUpListQuery($module)
        {
            $mod=CRMEntity::getInstance($module);
            
            $query = "SELECT vtiger_crmentity.*, $mod->table_name.*";

            // Keep track of tables joined to avoid duplicates
            $joinedTables = array();

            // Select Custom Field Table Columns if present
            if(!empty($mod->customFieldTable)) $query .= ", " . $mod->customFieldTable[0] . ".* ";

            $query .= " FROM $mod->table_name";

            $query .= "	INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = $mod->table_name.$mod->table_index";

            $joinedTables[] = $mod->table_name;
            $joinedTables[] = 'vtiger_crmentity';

            // Consider custom table join as well.
            if(!empty($mod->customFieldTable)) {
                $query .= " INNER JOIN ".$mod->customFieldTable[0]." ON ".$mod->customFieldTable[0].'.'.$mod->customFieldTable[1] .
                          " = $mod->table_name.$mod->table_index";
                $joinedTables[] = $mod->customFieldTable[0]; 
            }

            $linkedModulesQuery = $this->db->pquery("SELECT distinct fieldname, columnname, relmodule,reltablename FROM vtiger_field" .
            ...
            for($i=0; $i<$linkedFieldsCount; $i++) {
                ...
                $reltablename = $this->db->query_result($linkedModulesQuery, $i, 'reltablename');
                ...
                vtlib_setup_modulevars($related_module, $other);
                            ...
                            if($reltablename=='')
                {
                    $reltablename=$this->table_name;
                }

                if(!in_array($other->table_name, $joinedTables)) {
                    $query .= " LEFT JOIN $other->table_name ON $other->table_name.$other->table_index = $reltablename.$columnname";
            ...
            $mod->where_clause_query['where_clause'] !='' ? $query .=$mod->where_clause_query['where_clause'] : ''; 
            
            return $query;
        }
        
         /**
         * Update Query for new updated time for main record also
         * 
         * @param CRMrecordId $id
         */
        
        function updateCrmModifiedDate($id, $module){
             global $adb;
             $sql = "UPDATE vtiger_crmentity SET modifiedtime = ? WHERE setype_id = ? AND crmid = ?"; 
             $setype_id = getTabid($module);
             $adb->pquery($sql , array(date('Y-m-d H:i:s', time()), $setype_id, $id));
             return true;
        }        
