...
class QueryGenerator {
	...
	public function getFromClause() {
		...
		$defaultTableList = $this->meta->getEntityDefaultTableList();
                //echo "<pre>";print_R($defaultTableList);print_r($tableList);
		...
		if( $this->meta->getTabName() == 'xWindowDisplayScheme' && !empty($distributorid)) {
			$tableJoinCondition['folderid'] = array(
				'vtiger_xdistributorcluster_mrel'=>"vtiger_crmentity.crmid = vtiger_xdistributorcluster_mrel.crmid"
			);
			$tableJoinMapping['vtiger_xdistributorcluster_mrel'] = 'INNER JOIN';
		}
		...
		foreach ($this->manyToManyRelatedModuleConditions as $conditionInfo) {
			...
			$relationInfo['relationTable'].".$relationInfo[$relatedModule]=".
                ...
                if($baseTable!="vtiger_xdistributorterms")
                    $sql .= $this->meta->getEntityAccessControlQuery();

                
                
                if($baseTable=="vtiger_troubletickets")
                {
                    $sql .= " <<<<<accountusermappingquery>>>>> ";
                }
                else if($baseTable=="vtiger_potential" || $baseTable=="vtiger_account" 
                        || $baseTable=="vtiger_leaddetails" || $baseTable=="vtiger_contactdetails"
                        || $baseTable=="vtiger_notes")
                {
                    $sql .= " <<<<<productmapping>>>>> ";
                }    
	...
	public function getWhereClause() {               
		...
		if($this->conditionInstanceCount > 0) {
			...
			$sql .= $groupSql;
                ...
                if($baseTable=="vtiger_notes" && !is_admin($current_user))
                {
                    $sql .= " and filelocationtype!='D'";
                }
	...
	private function getConditionValue($value, $operator, $field) {
            if($field->getTableName() == "vtiger_xsalesordercf" && $field->getColumnName()=="cf_xsalesorder_seller_id" && $operator=='h'){
                $operator = 'c';
            }
		...
		foreach ($valueArray as $value) {
			...
			if(!$this->isNumericType($field->getFieldDataType()) &&
				...
				$value = "'$value'";
                        ...
                        //Modify for 'DistributorId as Distributor Name' search issue in xClaimTopSheet & xClaimScheme module
                        if(($field->getTableName() == "vtiger_xclaimtopsheet" && $field->getColumnName()=="xdistributorid") || ($field->getTableName() == "vtiger_xclaimscheme" && $field->getColumnName()=="xdistributorid") || ($field->getTableName() == "vtiger_xsalesordercf" && $field->getColumnName()=="cf_xsalesorder_seller_id") ){
                            $value = "'$value'";
                        }
	...
	public function addUserSearchConditions($input) {
		...
		if($input['searchtype']=='advance') {
			...
			$this->endGroup();
                }else if($input['searchtype']=='kadvance') {
                        if(empty($input['search_cnt'])) {
				return ;
			}
			$noOfConditions = vtlib_purify($input['search_cnt']);
			...
			if($this->conditionInstanceCount > 0) {
				...
				$this->startGroup('');
                        ...
                        $ngrs=vtlib_purify($input['grp_count']);
                        //print_r($ngrs);
                        $listCond=explode(":",$ngrs);  
                        //print_r($listCond);
                                                
			for($j=0; $j<count($listCond); $j++) {
				if($listCond[$j]=='gsand')
                                {
                                    $this->startGroup(self::$AND);
                                }
				else if($listCond[$j]=='gsor')
                                {
                                    $this->startGroup(self::$OR);
                                }
				else if($listCond[$j]=='gs')
                                {
                                    $this->startGroup('');
                                }
                                else if($listCond[$j]=='ge')
                                {
                                    $this->endGroup();
                                }   
                                else if($listCond[$j]=='and')
                                {
                                    $this->addConditionGlue(self::$AND);
                                }   
                                else if($listCond[$j]=='or')
                                {
                                    $this->addConditionGlue(self::$OR);
                                }   
                                else if(trim($listCond[$j])!=""){
                                    $i=$listCond[$j];
                                    $fieldInfo = 'Fields'.$i;
                                    $condition = 'Condition'.$i;
                                    $value = 'Srch_value'.$i;
